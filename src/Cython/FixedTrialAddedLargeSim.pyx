...
cdef trial_progress(int nsim, int seed, int n_arm, int n_stage, double[::1] te_list, int[:, ::1] ps_array, float alloc, float ful, float eff, 
        ...
        for i in range(0, n_arm-1):
            ...
            for k in xrange(0, nsim):
                ...
                if interim_dict.has_key((success1, success2, fail1, fail2)):
                    ...
                    efficacy[i, j, k] = int(interim_dict[(success1, success2, fail1, fail2)] > efficacy)
        ...
        if j < n_stage -1:
            for ink in xrange(0, nsim):
                ...
                if tps_stop == n_arm-1:
                    ...
                    tps_trt = np.rint(tps_stage * 1/(n_arm-1 - tps_stop + alloc))
                    tps_control = tps_stage - tps_trt * (n_arm -1 - tps_stop)
                    for ini in range(0, n_arm-1):
                        if stop[ini, j, ink] != 1:
                            tps_array[ini, j+1, ink] =  tps_array[ini, j, ink] + (ps_array[ini, j+1] + tps_trt)*(1-stop[ini, j, ink]) 
                        else: 
                            tps_array[ini, j+1, ink] = 0
                    tps_array[n_arm-1, j+1, ink] = tps_array[n_arm-1, j, ink] + ps_array[n_arm-1, j+1] + tps_control
        ...
        if j == n_stage - 1 and pred_flag == 1:
            ...
            for ink in xrange(0, nsim):
                ...
                if tps_stop != n_arm-1:
                    ...
                    pred_trt_add = np.rint(pred_num * 1/(n_arm-1-tps_stop + alloc))
                    pred_control_add = pred_num - pred_trt_add * (n_arm -1-tps_stop)
                    for ini in range(0, n_arm-1):
                        pred_trt[ini, ink] = pred_trt_add*(1-stop[ini, j, ink])
    ...
    cdef float *prob_trt_pointer = <float *>malloc(pred_num_trt * sizeof(float))
    cdef float[::1] prob_trt = <float[:pred_num_trt]> prob_trt_pointer
    ...
    if pred_flag == 1:
            ...
            for ck in xrange(0, nsim):
                ...
                if stop[ci, n_stage-1, ck] == 1:
                    ...
                    for cm in xrange(0, pred_trt[ci, ck] + 1):
                        ...
                        prob_trt[cm] =  gsl_sf_choose(beta_index1, beta_index2) * gsl_sf_choose(beta_index3, beta_index4)/gsl_sf_choose(pred_trt_total + 1, pred_trt[ci, ck])                   
                    ...
                    if pred_total_dict.has_key((obs_succ_control, obs_succ_trt, pred_control[ck], pred_trt[ci, ck])):
                        ...
                        for cm in xrange(0, pred_trt[ci, ck] + 1):
                            ...
                            for cl in xrange(0, cm_copy):
                                cpower_sum += prob_control[cl, ck] * prob_trt[cm]
        ...
        for ci in range(0, n_arm-1):
            con_power[ci] = np.sum(np.array(cpower_array[ci, :]))/(nsim - np.sum(np.array(stop[ci, n_stage-1, :])))*100   
    ...
    return con_futile_mean, con_efficacy_mean, con_continuous_mean, con_power
...
def trial_simulation(nsim, seed, n_arm, n_stage, te_list, ps_array, alloc, add_flag, ful, eff, clin_sig, pred_flag, *args):
    ...
    futile, efficacy, continuous, cpower = trial_progress(nsim, seed, n_arm, n_stage, te_list, ps_array, alloc, ful, eff, clin_sig,
