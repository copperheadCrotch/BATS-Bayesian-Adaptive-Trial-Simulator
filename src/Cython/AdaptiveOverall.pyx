        ...
        if j < n_stage -1:
            ...
            for ink in xrange(0, nsim):
                ...
                if tps_stop == n_arm-1:
                    ...
                    tps_trt = np.rint((tps_stage + tps_array[n_arm - 1, j, ink] + max(tps_array[0:n_arm - 2, j, ink])) * 1/(n_arm-1 - tps_stop + tps_alloc)) - max(tps_array[0:n_arm - 2, j, ink])
        ...
        if j == n_stage - 1 and pred_flag == 1:
            ...
            for ink in xrange(0, nsim):
                ...
                if tps_stop != n_arm-1:
                    ...
                    pred_trt_add = np.rint((pred_num + tps_array[n_arm - 1, j, ink] + max(tps_array[0:n_arm - 2, j, ink])) * 1/(n_arm - 1 - tps_stop + tps_alloc)) - max(tps_array[0:n_arm-2, j, ink])
